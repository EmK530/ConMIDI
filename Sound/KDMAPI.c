...
int KDMAPI_Setup(){
    ...
    if ((KDMAPI_SendDirectData = (KDM_SEND)GetProcAddress(KDMAPI_libHandle, "SendDirectData")) == NULL){printf("GetProcAddress failed for KDMAPI SendDirectData.\n");return 0;}
    if ((KDMAPI_SendDirectLongData = (KDM_LSEND)GetProcAddress(KDMAPI_libHandle, "SendDirectLongData")) == NULL){printf("GetProcAddress failed for KDMAPI SendDirectLongData.\n");return 0;}
    if ((KDMAPI_PrepareLongData = (KDM_LSEND)GetProcAddress(KDMAPI_libHandle, "PrepareLongData")) == NULL){printf("GetProcAddress failed for KDMAPI PrepareLongData.\n");return 0;}
    if ((KDMAPI_UnprepareLongData = (KDM_LSEND)GetProcAddress(KDMAPI_libHandle, "UnprepareLongData")) == NULL){printf("GetProcAddress failed for KDMAPI UnprepareLongData.\n");return 0;}
